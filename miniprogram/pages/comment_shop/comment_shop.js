...
Page({
  ...
  onLoad: function (options) {
    ...
    .then(res => {
        ...
        this.setData({
            shop_assessment: res.data[0].assessment, //
        })
        console.log(this.data.shop_assessment);
    })
    .catch(err => {
    ...
    db.collection('store_detail_list').where({shop_id:options}).get({
      success:function(res){
        console.log('load succeeded',res)
        that.setData({
          store_detail_list:res.data,
          id:options.id
        })
      },fail:function(res){
